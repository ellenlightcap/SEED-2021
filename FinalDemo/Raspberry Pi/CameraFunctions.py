...
def getAngle(img, height, width, corners, ids, currentMarker):
    #finding angle
    FIELDOFVIEW = 54
    ...
    if (ids != None):
        ...
        for marker in range(0,ids.size):
            ...
            if (currentMarker == ids[marker]):
                ...
                print("did not find desired marker")
...
def getDistance(img, height, width, corners, ids, currentMarker):
    ...
    if (ids != None):
        for marker in range(0,ids.size):
            #only finds distance for desired marker
            if (currentMarker == ids[marker]):
                mark = np.array(corners[marker][0])
